...
- **A forwarded `nil` option was used as the value instead of meaning "not set".** This
  ...
  `:depth` on the order book, and the fake's `:client_transfer_id` — which, `nil`, gave a
  fake withdrawal an id of `nil` where the real package always generates one — are
